var PIE = function(selector, context) {
	var element;
	if (typeof selector == 'string') {
		...
		selector.split(/, ?/).each(function(single) {
			if (/^[a-z]+$/.test(single)) {
				PIE.array(document.getElementsByTagName(single)).each(function(tagElement) {
					PIE.element(tagElement);
...
PIE.set = function(instance, properties) {
...
PIE.set(PIE, {
	...
	element: function(instance) {
		PIE.set(instance, {
			html: PIE.dual('innerHTML'),
			display: PIE.dual('style.display'),
			show: function() {
				this.display('');
			},
			hide: function() {
	...
	ajax: function(method, url, options) {
		if (typeof options == 'function') {
			...
			options = {ok: function(response) {
				PIE(id).html(response.responseText);
			}};
		}
		...
		PIE.set(headers, options.headers);
	...
	get: function(url, options) {
		PIE.ajax('GET', url, options);
	},
	post: function(url, options) {
		PIE.ajax('POST', url, options);
	}
});

PIE.set(Object.prototype, {
